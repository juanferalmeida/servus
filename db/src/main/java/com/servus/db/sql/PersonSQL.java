...
public class PersonSQL {
	...
	public boolean insert(Person pers) throws SQLException {
		...
		if (connection != null) {
			...
			ps = connection.prepareStatement(
					"INSERT INTO Person (userId, type, name, mobile, country, city, address, map, birthday) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ? ) ;");
	...
	public boolean modify(Person pers) throws SQLException {
		...
		if (connection != null) {
			PreparedStatement ps = null;
			ps = connection.prepareStatement(
					"UPDATE   Person  SET  type = ?, name = ?, mobile = ?, country = ?, city = ?, address = ? , map = ?, birthday = ? WHERE userId = ? ;");
