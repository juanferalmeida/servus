...
public class ReserveSQL {
	...
	public boolean insert(Reserve reserve) throws SQLException {
		...
		if (connection != null) {
			...
			ps = connection.prepareStatement(
					"INSERT INTO Reserve (reserveId, userId, serviceId, reserveDate, reserveStart, reserveEnd, status, score) VALUES(?, ?, ?, ?, ?, ?, ?, ? ); ");
	...
	public boolean modify(Reserve reserve) throws SQLException {
		...
		if (connection != null) {
			PreparedStatement ps = null;
			ps = connection.prepareStatement(
					"UPDATE   Person  SET  type = ?, name = ?, mobile = ?, country = ?, city = ?, address = ? , map = ?, birthday = ? WHERE reserveId = ? ;");
