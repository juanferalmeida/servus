...
public class ServiceSQL {
  ...
  public boolean insert(Service service) throws SQLException {
		...
		if (connection != null) {
			   PreparedStatement ps = null;
			 ps = connection.prepareStatement( "INSERT INTO Service (serviceId, adminId, serviceType, name, description, mobile, country, city, address, map, available) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ? ,? ); ");
	...
	public boolean modify(Service service) throws SQLException {
		...
		if (connection != null) {
			   PreparedStatement ps = null;
				 ps = connection.prepareStatement( "UPDATE   Service  SET  adminId = ?, serviceType = ?, name = ?, description = ?, mobile = ?, country = ? , city = ?, address = ?, map = ?, available= ? WHERE serviceId = ? ;" );
